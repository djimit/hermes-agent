...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if url.endswith("/anthropic"):
        ...
        if base_url_host_matches(url, "open.bigmodel.cn"):
            ...
            logger.debug("Auxiliary client: rewrote ZAI base URL")
            return rewritten
        rewritten = url[: -len("/anthropic")] + "/v1"
        logger.debug("Auxiliary client: rewrote Anthropic base URL")
        return rewritten
    if base_url_host_matches(url, "api.kimi.com") and url.endswith("/coding"):
        ...
        rewritten = url + "/v1"
        logger.debug("Auxiliary client: rewrote Kimi base URL")
        return rewritten
    return url
